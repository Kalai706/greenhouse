...
class Index extends CommonClass
	{
		...
		public function getRefresh()
			{
				...
				foreach($this->CFG['site']['grid']['home'] as $key=>$val)
					{
					?>
						<!-- <li style="box-sizing:;"><span>Garden<?php echo $i?></span><a href="javascript:void(0)" id="refreshrecord<?php echo $i;?>" onclick="refreshrecords('<?php echo $val;?>');">Refresh</a></li> -->
		...
		public function displayWatchListData($table_name ='',$search_query){
			...
			$row = $this->getTabelRecords($table_name,$this->CFG['site']['home']['limit'],false,'LAST_UPDT_ON','DESC',$whr_condition);
		...
		public function getWatchListData($watch_id){
			...
			$row = $this->getTabelRecords($this->CFG['db']['tbl']['watch_list'],$this->CFG['site']['home']['limit'],false,'IdWatchList','DESC',$whr_condition);	
		...
		public function getCaptureNote(){
				if($this->fields_arr['search_query'] != '')
					...
					$where_cond = '';
				$row = $this->getTabelRecords($this->CFG['db']['tbl']['notes'],$this->CFG['site']['home']['limit'],false,'LAST_UPDT_ON','DESC',$where_cond);				
