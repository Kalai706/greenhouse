				...
				foreach($this->CFG['site']['grid']['home'] as $key=>$val)
					{
						...
						$row = $this->getTabelRecords($val,$this->CFG['site']['home']['limit'],false,false,"DESC",false,false,"dbobj2","garden_details_nine");	
						if($row == ''){
		...
		public function getAjaxSearchPlanet()
			{
				...
				$row = $this->getTabelRecords($this->fields_arr['table_name'],$this->CFG['site']['home']['limit'],false,'LAST_UPDT_ON','DESC',$where_cond,false,'dbObj2','garden_details_nine');
				if($row){
